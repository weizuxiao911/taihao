...
KERNEL_BUILD_PROJECT_ROOT="$(cd "$KERNEL_BUILD_ROOT/.." && pwd)"
KERNEL_BUILD_REPO_ROOT="$(cd "$KERNEL_BUILD_PROJECT_ROOT/.." && pwd)"

# 默认路径(可被环境变量覆盖)
KERNEL_BUILD_LINUX_SRC="${KERNEL_BUILD_LINUX_SRC:-$KERNEL_BUILD_REPO_ROOT/linux-6.6}"
KERNEL_BUILD_OUT="${KERNEL_BUILD_OUT:-$KERNEL_BUILD_PROJECT_ROOT/out}"
KERNEL_BUILD_FRAG_DIR="${KERNEL_BUILD_FRAG_DIR:-$KERNEL_BUILD_REPO_ROOT/config/kernel}"
